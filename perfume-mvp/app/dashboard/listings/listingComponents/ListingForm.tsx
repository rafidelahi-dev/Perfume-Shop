...
type IntactPayload = BasePayload & {
  type: "intact";
  bottle_size_ml: number;
  partial_left_ml: null;
  decant_options: DecantOptionPayload[];
  price: number;
};

type PartialPayload = BasePayload & {
  type: "partial";
  partial_left_ml: number;
  bottle_size_ml: null;
  decant_options: DecantOptionPayload[];
  price: number;
};

type DecantPayload = BasePayload & {
  type: "decant";
  decant_options: DecantOptionPayload[];
  price: null;
  bottle_size_ml: null;
...
type ListingPayload = IntactPayload | PartialPayload | DecantPayload;
...
const ListingForm: React.FC = () => {
  ...
  const create = useMutation<void, Error, ListingPayload>({
    mutationFn: async (values) => {
      await insertListing(values);
    },
    onSuccess: () => {
  ...
  function validate(): string | null {
    ...
    if (type === "intact") {
      if (!bottleSize || bottleSize <= 0) {
        return "Bottle size (ml) is required for Intact.";
      }
    ...
    if (type === "partial") {
      if (!partialLeft || partialLeft <= 0) {
        return "Amount left (ml) is required for Partial.";
      }
      if (!singlePrice || singlePrice <= 0) {
        return "Price is required.";
      }
    ...
    if (type === "decant") {
      const valid = decants.filter(
        (d) => d.size_ml && d.price && d.size_ml > 0 && d.price > 0
      );
      if (valid.length === 0) {
        return "Add at least one decant size with price.";
      }
    }
    ...
    try {
      ...
      if (type === "intact") {
        ...
        payload = {
          ...base,
          type: "partial",
          partial_left_ml: Number(partialLeft),
          price: Number(singlePrice),
          bottle_size_ml: null,
          decant_options: [],
        };
        ...
        // type === "decant"
        const cleaned: DecantOptionPayload[] = decants
          .map((d) => ({
            ml: d.size_ml === "" ? null : Number(d.size_ml),
            price: d.price === "" ? null : Number(d.price),
          }))
          .filter(
            (d): d is DecantOptionPayload =>
              typeof d.ml === "number" &&
              d.ml > 0 &&
