...
import { useAdminBlogCategories, useAdminBlogTags, useDashboardUpdateBlogPost, type BlogPost } from '@/lib/queries/blog'
...
export default function DashboardEditBlogPage({ params }: Params) {
  ...
  if (!post) return <div className="text-center py-20 text-red-500">Post not found or not editable.</div>

  const canEdit = ['draft', 'rejected'].includes(post.status)
      ...
      <fieldset disabled={!canEdit} className="space-y-4">
        ...
        <div>
          ...
          <input value={slug} onChange={(e) => setSlug(e.target.value)}
            className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-gray-400 disabled:bg-gray-50" />
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Excerpt</label>
          <textarea rows={2} value={excerpt} onChange={(e) => setExcerpt(e.target.value)}
            className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-gray-400 resize-none disabled:bg-gray-50" />
        </div>
