...
export default function SignupPage() {
  ...
  const normalizedUsername = useMemo(
    () => username.toLowerCase().replace(/[^a-z0-9_]/g, "").slice(0, 20),
    [username]
  );

  useEffect(() => {
    setUsername(normalizedUsername);
  }, [normalizedUsername]); // keep the input sanitized as the user types
  ...
  async function checkAvailability(u: string) {
    ...
    const { data, error } = await supabase
      .from("profiles")
      .select("id")
      .ilike("username", u) // case-insensitive; we also have the lower() unique index
      .limit(1);
    setChecking(false);
    ...
    <div className="max-w-sm mx-auto rounded-xl border bg-white p-6">
      ...
      <form onSubmit={onSubmit} className="space-y-3">
        ...
        <input
          ...
          required
        />
        {checking && <p className="text-xs text-gray-500">Checking username…</p>}
        {usernameOk === false && <p className="text-xs text-red-600">Username is taken</p>}
        {usernameOk === true && <p className="text-xs text-green-700">Username available</p>}
